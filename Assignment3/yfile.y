...
SymbolTable table(49);

void yyerror(char *s)
{
	syntactical_error++;
	fprintf(error_out, "line no. %d: Syntactical error no. %d found\n%s\n", line_count, syntactical_error, s);
}
...
declaration_list : declaration_list COMMA ID
	{
		...
		symbolInfo = table.srch($3->getName());
		symbolInfo->varType = spType;
...
variable : ID
	...
	| ID LTHIRD expression RTHIRD 
	{
		if($3->varType != "int")
		{
			fprintf(error_out, "line no. %d: Semantic error no. %d found\nArray indexing error\n\n", line_count, semantic_error);
		}
